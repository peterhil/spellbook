...
import { get, map, nub, pick } from 'fkit'
...
export const getRecentCategories = async (count) => {
  const bookmarks = await getRecent(count)
  const ids = nub(map(getParentId, bookmarks))
